...
const OutstandingCreditAutocomplete = forwardRef((props,ref) => {
    const {allOption, selectedValues=[], isFormikFormField = false, showFormError = false, ...rest} = props;
    const outstandingCreditOptions = [{label:'Yes',id:1},{label:'No',id:2}]
    if(allOption){
        outstandingCreditOptions.unshift({label:'All', id:-1})
    }
    let initialProps = {
        ...rest,
        labelKey:'label',
        primaryKey:'id',
        options:outstandingCreditOptions,
        isObject:true,
        isFormikFormField,
        showFormError,
    ...
    useEffect(() => {
        if(selectedValues.length > 0){
            ...
            if(selectedValues.length > 0){
                preSelectedValues =  outstandingCreditOptions.filter((s) => selectedValues.indexOf(s.id) >= 0);
...
export default OutstandingCreditAutocomplete;