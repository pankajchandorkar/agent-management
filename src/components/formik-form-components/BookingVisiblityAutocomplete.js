...
const BookingVisiblityAutocomplete = forwardRef((props, ref) => {
    const { allOption, selectedValues = [], isFormikFormField = false, showFormError = false, ...rest } = props;
    const visiblityOptions = [
        { label: 'Own Booking', id: 1 },
        { label: 'Branch All Booking', id: 2},
        { label: 'Branch Restricted', id: 3 }
    ]
    if (allOption) {
        visiblityOptions.unshift({ label: 'All', id: -1 })
    }
    let initialProps = {
        ...rest,
        labelKey: 'label',
    ...
    useEffect(() => {
        if (selectedValues.length > 0) {
            ...
            if (selectedValues.length > 0) {
                preSelectedValues = visiblityOptions.filter((s) => selectedValues.indexOf(s.id) >= 0);
...
export default BookingVisiblityAutocomplete;