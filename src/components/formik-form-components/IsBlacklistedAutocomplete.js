...
const IsBlacklistedAutocomplete = forwardRef((props,ref) => {
    const {allOption, selectedValues=[], isFormikFormField = false, showFormError = false, ...rest} = props;
    const isBlacklistedOptions = [{label:'True',id:1},{label:'False',id:2}]
    if(allOption){
        isBlacklistedOptions.unshift({label:'All', id:-1})
    }
    let initialProps = {
        ...rest,
        labelKey:'label',
    ...
    useEffect(() => {
        if(selectedValues.length > 0){
            ...
            if(selectedValues.length > 0){
                preSelectedValues =  isBlacklistedOptions.filter((s) => selectedValues.indexOf(s.id) >= 0);
...
export default IsBlacklistedAutocomplete;